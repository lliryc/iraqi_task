...
punctuation_prompt = ChatPromptTemplate.from_template("""You are Iraqi Arabic speaker. Your task is to correct the punctuation in the Iraqi Arabic text below. Don't change words, just correct the punctuation. Return only the corrected text without any other explanation.
...
paragraphs_prompt = ChatPromptTemplate.from_template("""You are Iraqi Arabic speaker. Your task is to split Iraqi Arabic text into paragraphs if it makes sense. Otherwise, return the original text. Don't change words and punctuation. Return only the corrected text without any disclaimers.
...
if __name__ == "__main__":
    ...
    for file in tqdm(glob.glob("iraqi_videos_texts_check/*.txt")):
        ...
        with open(f"iraqi_videos_texts_punct_corrected/{file_name}", "w", encoding="utf-8") as f:
        ...
        if not processed:
            ...
            os.remove(f"iraqi_videos_texts_punct_corrected/{file_name}")
