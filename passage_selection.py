...
Extract passage from a source text in Iraqi Arabic for reading comprehension task. 
...
def get_processed_passages():
  with open("iraqi_processed_passages.txt", "r") as f:
    lines = f.readlines()
...
def add_processed_passages(files):
  with open("iraqi_processed_passages.txt", "a") as f:
    for file in files:
      f.write(file + "\n")
...
def extract_passage(source_text_file):
  ...
  if cnt_tokens <= 600:
    output_file = f"iraqi_passages/{filename}"
...
if __name__ == "__main__":    
    input_files = glob.glob("iraqi_videos_texts_punct_corrected/*.txt")
