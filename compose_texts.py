...
def get_transcripts_dict():
  ...
  transcripts = glob("iraqi_videos_transcripts/*.json")
...
def get_processed_videos():
  with open("iraqi_videos_transcripts_processed.txt", "r") as f:
    lines = f.readlines()
...
def write_processed_videos(video_ids):
  with open("iraqi_videos_transcripts_processed.txt", "a") as f:
...
segmentation_output_schema = {
  ...
  "title": "Iraqi Arabic Transcript Segmentation",
  "description": "A list of segmentation markers for Iraqi Arabic transcripts. Each object indicates punctuation mark is inserted next to text item with the given id",
  "type": "array",
  "items": {
    "type": "object",
    "properties": {
...
def compose_text(video_id):
  ...
  with open(f"iraqi_videos_texts/{video_id}.txt", "w") as f:
    ...
    for text_block in merged_transcript:
      ...
      while tries < 10:
        prompt1 = f"""Take Iraqi Arabic transcript in a JSON format and add punctuation marks between text items if needed by analyzing the text as well as start and end times. Return result in a JSON format according to output schema without any disclaimer.
  ...
  if tokens_exceeded:
    ...
    os.remove(f"iraqi_videos_texts/{video_id}.txt")
...
def compose_text2(video_id):
  ...
  with open(f"iraqi_videos_texts2/{video_id}.txt", "w") as f:
    merged_transcript = merge_text_blocks_by_speaker_id(enriched_transcript_6min)
