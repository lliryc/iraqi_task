...
def presample_videos(sample_size):
    cdf = pd.DataFrame()
    for file in os.listdir("iraqi_channels_videos"):
        df = pd.read_csv(f"iraqi_channels_videos/{file}")
    ...
    print(len(cdf))
    cdf.to_csv("iraqi_channels_videos_presampled.csv", index=False)
...
def get_transcripts_metadata():
    ...
    with open(os.path.join(script_directory, "iraqi_channels.txt"), "r") as playlist_file:
    ...
    with open(os.path.join(script_directory, "iraqi_channels_processed.txt"), "r") as processed_file:
    ...
    with open(os.path.join(script_directory, "iraqi_channels_processed.txt"), "a") as processed_file:
        ...
        for i, playlist_url in enumerate(playlist_urls):
            ...
            try:
              ...
              lev_channel_df.to_csv(os.path.join(script_directory, "iraqi_channels_videos", f"{channel_id}.csv"), index=False)
